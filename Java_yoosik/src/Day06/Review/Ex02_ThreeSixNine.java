package Day06.Review;

import java.util.Iterator;

public class Ex02_ThreeSixNine {
	
	public static void main(String[] args) {
		// 정수 1~100 까지 수를 반복하여 출력하면서,
		// 해당 수가 3 또는 6 또는 9가 될 때는
		// 자리 수 마다 3,6,9가 되는 개수 만큼 정수 대신
		// "*" 을 출력하는 프로그램을 작성하시오.
		// ex)
		// 13 : *
		// 66 : **
		
		// 순서도
		// 1. 정수 1~100까지 출력
		int a = 0;
		for (int i = 1; i <= 100; i++) {
			// 2. 해당 수에 3,6,9가 몇 개 인지 판단 / 출력된 수에 3,6,9가 들어 있는지 확인
			
			// i : 1~100
			// ex) i: 16
			// 십의 자리수 : 해당수(i) / 10의 정수부분
			// 일의 자리수 : 해당수(i) % 10 <--10으로 나눈 나머지
			int ten = i / 10;
			int one = i % 10;
			
			
			// 2-1. 십의 자리수가 3,6,9인지 여부 판단 (A)
			// 		일의 자리수가 3,6,9인지 여부 판단 (B)
//			boolean ten369 = (ten == 3 || ten  == 6 || ten == 9);
//			boolean one369 = (ten == 3 || ten  == 6 || ten == 9);
			boolean ten369 = (ten != 0 && ten % 3 == 0);
			boolean one369 = (one != 0 && one % 3 == 0);
			
			// 3. 3,6,9 개수에 따라 출력
			// 	  A and B --> **
			//    A or B --> *
			// 	  그 외 --> 정수
			if (ten369 && one369) {
				System.out.println("**");
			}
			else if (ten369 || one369) {
				System.out.println("*");
			}
			else {
				System.out.println(i);
			}
			
		}
		
		
		
		
	}
}
