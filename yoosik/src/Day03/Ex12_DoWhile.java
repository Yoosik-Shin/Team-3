...
public class Ex12_DoWhile {
	...
	public static void main(String[] args) {
		...
		// 0. 종료
		// 메뉴번호 : 입력받기
		// 종료(0번) 누를때까지 메뉴판 계속 실행
		// * 변수 선언 : 메뉴번호, 메뉴이름
		int menuNo = 0;     // 메뉴번호
		String menuName = "";     //메뉴이름
		String menuCount = "";      // 메뉴이름추가
		int count = 0;     // 메뉴개수
		...
		do {
			...
			if(menuNo >= 0 && menuNo <= 5) {
				...
				menuCount = menuCount + menuName;
			} else {
				System.out.println("(0~5)번 사이의 번호를 입력해 주세요.");
			}
			System.out.println();
			
		} while (menuNo != 0);
		System.out.println(menuCount + ", " + count + "개의 메뉴를 주문하였습니다.");
